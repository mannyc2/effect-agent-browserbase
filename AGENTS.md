...
- Owned code follows the strict policy in `lint/owned.ts` and the owned tsconfigs; `CONTRIBUTING.md` describes it. Fix a lint finding or Effect diagnostic rather than suppress it. A genuine exception states its reason, and acceptance rejects a directive that no longer suppresses anything.
