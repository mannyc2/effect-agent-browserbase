...
This design avoids speculative caches and parallel browser workers. Playwright [does not generally recommend caching browser binaries](https://playwright.dev/docs/ci#caching-browsers), while Vite+ [requires measuring task-cache restore/save cost and stable fingerprints](https://viteplus.dev/guide/github-actions-cache). Removing unrelated work and duplicate executions comes first. A future cache must demonstrate a net improvement without reusing success for native side effects.
