...
The scheduled all-suite canary also needs PostgreSQL for upstream adapter certification. CI starts an isolated `postgres:17-alpine` container for that run and removes it afterwards. To reproduce locally, start the fixture with `docker run --detach --name effect-agent-test-postgres --publish 127.0.0.1:55432:5432 --env POSTGRES_PASSWORD=postgres --env POSTGRES_DB=effect_agent postgres:17-alpine`, wait for `docker exec effect-agent-test-postgres pg_isready -U postgres -d effect_agent` to succeed, then run `BROWSERBASE_UPSTREAM_TESTS=all bash tools/run-acceptance.sh full`. Remove it afterwards with `docker rm --force effect-agent-test-postgres`. An existing disposable test server can instead be supplied through `EFFECT_AGENT_TEST_POSTGRES_URL`; its user must be able to create and drop temporary databases. Default full acceptance tests only the owned workspaces and needs no database, as do the library and documentation profiles.
...
| `library` | Tooling, frozen bootstrap, early canonical format/lint and all three package types, all package unit suites/builds, exports/purity, all three candidate tarballs, all five strict consumers and every generic/Agent native test from those tarballs, release identity and the three-package dry-run                                                          | Owned package, tooling, workflow and media changes                                                                                         |
| `full`    | All library checks plus separate source-native suites, the pinned upstream workspace's whole `check` and `build`, its `test` for the three owned packages (every upstream workspace in the scheduled canary), and the upstream release dry-run                                                       | Integration/pin/bootstrap changes, unknown paths, unavailable/empty diff, scheduled integration, default manual and reusable release calls |
