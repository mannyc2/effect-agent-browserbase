...
Full acceptance also needs PostgreSQL for the pinned upstream adapter certification. CI starts an isolated `postgres:17-alpine` container only for that profile and removes it afterwards. Locally, start the same fixture with `docker run --detach --name effect-agent-test-postgres --publish 127.0.0.1:55432:5432 --env POSTGRES_PASSWORD=postgres --env POSTGRES_DB=effect_agent postgres:17-alpine`, wait for `docker exec effect-agent-test-postgres pg_isready -U postgres -d effect_agent` to succeed, then run full acceptance. Remove it afterwards with `docker rm --force effect-agent-test-postgres`. An existing disposable test server can instead be supplied through `EFFECT_AGENT_TEST_POSTGRES_URL`; its user must be able to create and drop temporary databases. The library and documentation profiles do not need this service.
