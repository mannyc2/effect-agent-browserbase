...
The `0.2.0-beta.2` package set was released from tag `v0.2.0-beta.2` through the [release workflow](docs/RELEASING.md), with provenance, on npm's `beta` dist-tag, after `0.2.0-beta.0` and `0.2.0-beta.1`; `main` may be ahead of it. Install by exact version or `@beta`: a prerelease never moves `latest`, which still points at a name reservation (`0.0.0-reserved.0`) for `effect-browser` and `effect-agent-browser` and at `0.1.0-beta.102` for `effect-browserbase`. The former two-package graph (`effect-browserbase` and `effect-agent-browserbase`) ended with `0.1.0-beta.104`.
