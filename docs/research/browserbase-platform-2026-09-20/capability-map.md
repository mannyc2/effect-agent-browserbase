...
| Project CA resources | **Absent.** SDK has resource support and proxySettings references. [SDK resources][sdk] | Reference pass-through Stage 1; bounded administrative CRUD later only with workflow demand. Medium enterprise value |
| Project metadata/usage/quota/admission | **Absent** beyond HTTP status/retry-after translation. [HTTP][http] | Passive administrative inspection/optional host admission, not a mandatory scheduler or stale plan table. No assertion that existing clients own rate schedulers |
...
| Workflow/capability | Current support/evidence | Recommended module / stage |
| --- | --- | --- |
| Session logs | **Absent** and disabled at creation. [Provider][provider], [SDK logs][sdk] | G Logs and Launch, Stages 1/5; distinguish provider/CDP logs, native console signals and application lifecycle events |
...
| Session/Context webhooks | **Outside available browser contract.** SDK webhook events are Functions-only. [SDK webhooks][webhooks] | No speculative flush/session webhook design; bounded polling until provider browser events exist |
| Search, inference, autonomous agents, Functions deployment | **Intentionally outside browser integration exercise** | No dependencies required for either package |
