...
import { Chromium, type ChromiumCleanupResult } from "../src/Chromium.ts";
  ...
  Effect.scoped(
    Effect.gen(function* () {
      for (const interrupt of [false, true]) {
        ...
        yield* Effect.scoped(
          Effect.gen(function* () {
            const acquired = yield* (yield* Chromium).acquire(BrowserPolicy.unrestricted());
...
it.live(
  ...
  () =>
    Effect.scoped(
      Effect.gen(function* () {
        const result = yield* (yield* Chromium)
          .launch(BrowserPolicy.unrestricted())
          .pipe(Effect.result);
