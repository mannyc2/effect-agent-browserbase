...
describe.each(["node", "bun"])("%s Chromium host lifetime", (runtime) => {
  describe.each(["acquired", "connected"])("%s", (phase) => {
    describe.each([
      { stop: "close", knownFailure: false },
      { stop: "exit", knownFailure: true },
      { stop: "SIGTERM", knownFailure: true },
      { stop: "SIGKILL", knownFailure: true },
    ] as const)("$stop", ({ stop, knownFailure }) => {
      ...
      beforeAll(async () => {
        ...
        if (stop === "SIGTERM" || stop === "SIGKILL") assert.ok(host.kill(stop));
        ...
        assert.equal(host.signalCode, stop === "SIGTERM" || stop === "SIGKILL" ? stop : null);
        assert.equal(host.exitCode, stop === "close" ? 0 : stop === "exit" ? 23 : null);
