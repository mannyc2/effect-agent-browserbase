...
`BrowserbaseProjects.usage` is the passive project read for the provider's `browserMinutes` and `proxyBytes`. Its `projectId` is attached from the Client's configured project; the usage reply does not echo it. The reply has no billing-period or provider-as-of field. The [usage observation example](examples/usage-observation.ts) reads it, takes an Effect Clock timestamp **after** the decoded reply, and passes a credential-free sample to a host-owned persistence effect. That timestamp means “observed by this host,” not “current in Browserbase's ledger.” A difference between two samples is an observed counter change; reporting lag, adjustment and reset behavior are unspecified. `BrowserbaseSessions` can show session start/end times and proxy bytes, but those fields do not establish billable minutes or bandwidth.
...
The smallest local operation seam today is an **opt-in host wrapper** around the public Search, Fetch and Agent-run Effects. The example builds each operation from the exact Client whose project it journals, then uses `Effect.acquireUseRelease` to commit a host-journal intent before the POST and settle it on exit. The host must supply a bounded, durable `begin` that stores the supplied Effect Clock start time with a unique id and an idempotent `finish`; a failed `begin` stops the call and adds its typed error, while an unsettled intent remains `unknown` after a crash. The start time places a pending intent in a local activity window; it is not a provider billing timestamp. The example gives cooperative settlement two seconds and logs a sanitized warning if it fails or times out. A sink that ignores interruption can defeat that deadline, and cancellation during settlement can still change the caller's final Exit. A post-call `Effect.onExit` alone cannot guarantee a durable intent.

A decoded API reply confirms only that reply; `PlatformError.outcome` distinguishes a proven local refusal (`undispatched`), provider refusal (`rejected`) and uncertainty (`unknown`). An unknown outcome does not prove that a request was sent. A rejected reply does not prove that no unit was billed. An interrupt or defect may have no typed `PlatformError` at all. The example's persisted observation fields retain only project, finite operation, optional Fetch format/proxy flags, time and outcome; they exclude the search query, Fetch URL or schema, Agent task, credentials, response body and native values. Effect spans can still export failure causes, so the host's journal and billing-verifier errors and defects must also be sanitized. No process-global metric, queue or library observer service is implied by this example.

An allowance needs a caller-supplied account/plan scope, meter and unit, included quantity, and explicit half-open billing period. The example runs an Effectful host provenance check with a five-second cooperative timeout, then reads assessment time from the Effect Clock. It returns `Unknown` unless a decoded billing record has that exact scope, meter, unit and period, a measurement cutoff, and a final or fresh to-date status. A verifier failure retains its typed `E` and `R`; timeout yields `Unknown`. Its Schema validates shape only; `source: "provider-billing"` is a claim in the record, not proof. The host must obtain and verify the record through an independent account billing channel. Imported billing timestamps must have an explicit UTC offset. In particular, neither a project usage sample nor 720 locally confirmed Fetch replies establishes “280 of 1,000 calls left”: the API has no billing-window marker or call totals. A host may separately alert on **locally observed activity** against its own threshold, clearly labeled as such. Effect metrics and spans can report finite operation/outcome/requested format/proxy values; keep project ids, journal ids, URLs, queries, tasks and timestamps out of metric labels. Metrics are not a durable allowance ledger.
