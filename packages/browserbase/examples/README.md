...
| [`usage-observation.ts`](usage-observation.ts)      | An Effect Clock timestamp after the raw project usage read, an opt-in host journal for Search/Fetch/Agent-run outcomes, and an allowance assessment that stays `Unknown` without matching provider billing evidence. No credentials or provider calls run on import.                         |
