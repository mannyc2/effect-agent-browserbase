...
const ContextName = Schema.NonEmptyString.check(
...
export class BrowserbaseContexts extends Context.Service<
  BrowserbaseContexts,
  {
    ...
    }) => Effect.Effect<ContextCreation, ContextError>;
    readonly retrieve: (
      reference: ContextReference,
  ...
  static readonly layer: Layer.Layer<BrowserbaseContexts, never, BrowserbaseClient> = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const create = Effect.fn("BrowserbaseContexts.create")(function* (
        ...
        const created = yield* Schema.decodeUnknownEffect(Schema.Struct({ id: Identifier }))(
          raw,
        ).pipe(Effect.mapError(() => malformed("context-create", true)));
