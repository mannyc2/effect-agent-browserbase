...
export const ProxyRule = Schema.Union([
  Schema.Struct({ type: Schema.Literal("browserbase"), domainPattern: Schema.optionalKey(BoundedString), geolocation: Schema.optionalKey(Geolocation) }),
  Schema.Struct({ type: Schema.Literal("external"), server: BoundedString, domainPattern: Schema.optionalKey(BoundedString), username: Schema.optionalKey(ProxySecret), password: Schema.optionalKey(ProxySecret) }),
...
export const ProviderLaunchOptions = Schema.Struct({
  region: Schema.optionalKey(Schema.Literals(["us-west-2", "us-east-1", "eu-central-1", "ap-southeast-1"])),
  proxies: Schema.optionalKey(Schema.Union([Schema.Boolean, Schema.Array(ProxyRule).check(Schema.isMaxLength(64))])),
  proxySettings: Schema.optionalKey(Schema.Struct({ caCertificates: Schema.optionalKey(Schema.Array(Identifier).check(Schema.isMaxLength(64), Schema.isUnique())) })),
  extensionId: Schema.optionalKey(Identifier),
  browserSettings: Schema.optionalKey(Schema.Struct({
    verified: Schema.optionalKey(Schema.Boolean),
    os: Schema.optionalKey(Schema.Literals(["windows", "mac", "linux", "mobile", "tablet"])),
    allowedDomains: Schema.optionalKey(Schema.Array(BoundedString).check(Schema.isMaxLength(64), Schema.isUnique())),
  ...
  userMetadata: Schema.optionalKey(Schema.Record(Schema.String.check(Schema.isMaxLength(128)), Schema.Json)),
});
...
export const LaunchRecipe = Schema.Struct({
  ...
  context: Schema.optionalKey(Schema.Struct({ reference: ContextReference, persist: Schema.Boolean })),
  viewport: Schema.Union([
    Schema.Struct({ _tag: Schema.Literal("ProviderManaged") }),
    Schema.Struct({ _tag: Schema.Literal("Fixed"), width: Viewport.fields.width, height: Viewport.fields.height }),
  ]),
