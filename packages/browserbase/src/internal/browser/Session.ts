import { Clock, Deferred, Effect, Redacted, Schema, Scope } from "effect";
  ...
  const acquired = yield* acquireRemote(
    {
      ...
      allocationDeadline: owner.lifetimeDeadline,
  ...
  const controls = {
    ...
    detach: owner
      .guard(
        ...
        (ticket) =>
          Effect.gen(function* () {
            if (options.launch.keepAlive !== true)
              return yield* BrowserError.make({
                operation: "detach",
                reason: "unsupported",
                outcome: "undispatched",
