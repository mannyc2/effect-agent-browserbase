 ...
 */
export const acquireSession = <L extends RemoteLease, E>(
  limits: Limits,
  ...
  Effect.gen(function* () {
    ...
    const capture: CaptureParent = {
      ...
      target: () => {
        if (driver === undefined)
          throw BrowserError.make({ operation: "target", reason: "closed" });
    ...
    const local: LocalCleanup = {
      ...
      disconnect: Effect.suspend(() => {
        ...
        if (acquired === undefined)
          return Effect.succeed<CleanupResult["local"]>(
            connectPending ? "pending" : "not-connected",
          );
    ...
    const controls = {
      ...
      closePage: (id: string) =>
        nativeOperation(
          "close-page",
          (driver, ticket) => driver.closePage(id, ticket),
          true,
          false,
        ),
