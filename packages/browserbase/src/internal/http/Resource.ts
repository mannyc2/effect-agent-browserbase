 ...
 */
export const resource = <E>(
  client: BrowserbaseClient["Service"],
  make: (failure: RequestFailure) => E,
) => {
  ...
  const malformed = (operation: string, mutation: boolean) =>
  ...
  const remove = (path: string, operation: string): Effect.Effect<void, E> =>
