...
const proxy = (rule: ProxyRule): Schema.Json => rule.type === "external" ? {
  type: rule.type, server: rule.server,
  ...
  type: rule.type, ...(rule.domainPattern === undefined ? {} : { domainPattern: rule.domainPattern }),
} : {
  type: rule.type, ...(rule.domainPattern === undefined ? {} : { domainPattern: rule.domainPattern }),
...
export const compileLaunch = Effect.fnUntraced(function* (input: unknown, attempt: AllocationAttempt): Effect.fn.Return<CompiledLaunch, BrowserError> {
  ...
  const viewport = recipe.viewport._tag === "Fixed"
    ? yield* Schema.decodeUnknownEffect(Viewport)({ width: recipe.viewport.width, height: recipe.viewport.height }).pipe(Effect.mapError(fail))
    : undefined;
  if (Array.isArray(recipe.provider.proxies)) {
    for (const rule of recipe.provider.proxies) {
      if (rule.type === "browserbase" && rule.geolocation?.state !== undefined && rule.geolocation.country !== "US") return yield* fail();
      if (rule.type === "external") {
        const valid = yield* Effect.try({
          try: () => { const url = new URL(rule.server); return ["http:", "https:"].includes(url.protocol) && !url.username && !url.password && !url.hash; },
          catch: fail,
        });
  ...
  for (const domain of settings.allowedDomains ?? []) {
    const valid = yield* Effect.try({
      try: () => { const url = new URL(`https://${domain}/`); return !domain.includes("*") && url.hostname === domain && !url.port && !url.username && !url.password; },
      catch: fail,
    });
    if (!valid) return yield* fail();
  }
  const metadata = recipe.provider.userMetadata ?? {};
  if (Object.keys(metadata).length > 32 || Object.keys(metadata).some((key) => key === "effectAgentAttempt" || key.startsWith("browserbaseIntegration"))) return yield* fail();
  const keepAlive = recipe.keepAlive ?? false;
  const context = recipe.context === undefined ? undefined : { id: recipe.context.reference.contextId, persist: recipe.context.persist };
  const recordSession = settings.recordSession ?? false;
  const body = {
    projectId: attempt.projectId, timeout: recipe.remoteTimeoutSeconds, keepAlive,
    ...(recipe.provider.region === undefined ? {} : { region: recipe.provider.region }),
    proxies: Array.isArray(recipe.provider.proxies) ? recipe.provider.proxies.map(proxy) : recipe.provider.proxies ?? false,
    ...(recipe.provider.proxySettings === undefined ? {} : { proxySettings: recipe.provider.proxySettings }),
    ...(recipe.provider.extensionId === undefined ? {} : { extensionId: recipe.provider.extensionId }),
    browserSettings: {
      ...settings, recordSession, logSession: settings.logSession ?? false, solveCaptchas: settings.solveCaptchas ?? false,
      ...(context === undefined ? {} : { context }),
      ...(viewport === undefined ? {} : { viewport: { width: viewport.width, height: viewport.height } }),
    },
