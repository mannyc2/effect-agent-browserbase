...
export const makeContextWriterPermit = (
  ...
  Effect.suspend(() => {
    if (active.has(key(reference)) || active.size >= 2048)
      return Effect.fail(
        ContextError.make({ operation: "writer", reason: "active", outcome: "undispatched" }),
      );
    ...
    const noActive = () =>
      [...attempts.values()].every(
        (entry) => entry.state === "rejected" || entry.state === "terminal",
      );
    ...
    const permit: ContextWriterPermitInternal = {
      ...
      recordAttempt: (attempt) =>
        Effect.suspend(() => {
          if (
            ...
            quarantined ||
            !noActive() ||
            attempt.projectId !== owned.projectId ||
            (attempts.size > 0 && persistence._tag === "Unconfirmed")
          ) {
            return Effect.fail(
              ContextError.make({
                operation: "writer-admit",
                reason: "active",
                outcome: "undispatched",
              }),
            );
          }
          if (attempts.has(attempt.attemptId) || attempts.size >= 64)
            return Effect.fail(
              ContextError.make({
                operation: "writer-admit",
                reason: "limit",
      ...
      recordSession: (attempt, session) => {
        ...
        if (
          ...
          session.projectId !== owned.projectId ||
          (record.session !== undefined && record.session.sessionId !== session.sessionId)
        ) {
          unknown();
      ...
      rejected: (attempt) => {
        ...
        if ([...attempts.values()].every((item) => item.state === "rejected"))
          persistence = { _tag: "NotRequested" };
      },
      uncertain: (attempt) => {
        const record = attempts.get(attempt.attemptId);
      ...
      completed: (attempt, cleanup) => {
        ...
        if (
          ...
          record.session.sessionId !== cleanup.reference.sessionId ||
          cleanup.reference.projectId !== owned.projectId
        ) {
          unknown();
        ...
        if (cleanup.observedStatus !== "COMPLETED") {
          unknown();
        ...
        Effect.suspend(() => {
          if (
            ...
            [...attempts.values()].some(
              (entry) =>
                entry.state === "terminal" && entry.cleanup?.observedStatus !== "COMPLETED",
            )
          ) {
            return Effect.fail(
              ContextError.make({
                operation: "writer-readback",
                reason: "active",
                outcome: "undispatched",
              }),
            );
      ...
      snapshot: Effect.sync(() =>
        WriterSettlementFacts.make({
          reference: owned,
          attempts: [...attempts.values()].map((entry) =>
            WriterAttemptFacts.make({
          ...
          persistence,
          disposition:
            quarantined || !noActive() || persistence._tag === "Unconfirmed"
              ? "quarantine"
              : "release",
        }),
      ),
...
export const closeContextWriterPermit = (
...
export const requireContextWriterPermit = (
  ...
  Effect.suspend(() => {
    ...
    return actual !== undefined &&
      active.get(key(reference)) === actual &&
      actual.reference.contextId === reference.contextId &&
      actual.reference.projectId === reference.projectId
