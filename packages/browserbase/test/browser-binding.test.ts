...
import { BrowserPolicy, Viewport } from "../src/BrowserData.ts";
import { BrowserbaseClient } from "../src/Client.ts";
import { BrowserError } from "../src/Errors.ts";
import { bindingImplementation, type ConnectRequest } from "../src/internal/browser/Binding.ts";
...
it.effect("a binding the package did not issue is refused before any provider request", () =>
  Effect.gen(function* () {
    let requests = 0;
    const forged = { _tag: "BrowserbaseBrowserBinding" } as const;
...
it.effect("the default engine needs no provision and every issued binding is frozen", () =>
  Effect.gen(function* () {
    ...
    assert.equal(binding._tag, "BrowserbaseBrowserBinding");
