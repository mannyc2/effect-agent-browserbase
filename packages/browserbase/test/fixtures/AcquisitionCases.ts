...
const recipe: LaunchRecipe = { remoteTimeoutSeconds: 60, keepAlive: true, viewport: { _tag: "ProviderManaged" }, provider: { region: "us-east-1", browserSettings: { verified: true } } };
...
const ref = SessionReference.make({ provider: "browserbase", projectId: account.projectId, sessionId: "session-1" });
const context = () => ContextReference.make({ provider: "browserbase", projectId: account.projectId, contextId: globalThis.crypto.randomUUID() });
const metadata = (status = "RUNNING", projectId = account.projectId) => ({
  id: "session-1", projectId, status, createdAt: "2026-09-20T00:00:00Z", startedAt: "2026-09-20T00:00:00Z",
  updatedAt: "2026-09-20T00:00:00Z", expiresAt: "2026-09-20T00:01:00Z", keepAlive: true, proxyBytes: 0, region: "us-east-1",
...
const fail = (operation: string) => BrowserError.make({ operation, reason: "provider", outcome: "unknown" });
const expectFailure = <A, E, R>(value: Effect.Effect<A, E, R>) => value.pipe(Effect.result, Effect.map((result) => {
...
const fixture = (scenario: Scenario = {}) => {
  ...
  const fetch: typeof globalThis.fetch = async (input, init) => {
    ...
    if (path === "/v1/sessions" && request.method === "POST") {
      order.push("allocate"); bodies.push(await request.clone().json());
    ...
    if (path === "/v1/sessions/session-1" && request.method === "POST") {
      order.push("release"); released = true;
      return scenario.release === undefined ? Response.json(metadata("RUNNING")) : scenario.release();
    }
    if (path === "/v1/sessions/session-1" && request.method === "GET") {
      order.push("status");
      return scenario.retrieve === undefined ? Response.json(metadata(released ? "COMPLETED" : "RUNNING")) : scenario.retrieve();
    }
    throw new Error(`Unexpected provider operation ${request.method} ${path}`);
  };
  const action = (step: NonNullable<Scenario["localFailure"]>) => Effect.suspend(() => {
    order.push(step);
    return scenario.localFailure === step ? Effect.fail(fail(step)) : Effect.void;
  });
  const local: LocalCleanup = {
    fence: Effect.sync(() => { order.push("fence"); if (scenario.localFailure === "fence") throw new Error("fence failpoint"); }),
    capture: action("capture"), initialization: action("initialization"),
    disconnect: action("disconnect").pipe(Effect.as("closed" as const)),
  };
  const run = <A, E, R>(program: Effect.Effect<A, E, R>) => program.pipe(
    Effect.provide(layers), Effect.provideService(FetchHttpClient.Fetch, fetch),
  );
  return { order, bodies, local, run };
};
import { FetchHttpClient } from "effect/unstable/http";

const testTime = <A, E, R>(program: Effect.Effect<A, E, R>) => Effect.scoped(Effect.gen(function* () {
...
export const acquisitionCases = [
  {
    ...
    run: Effect.gen(function* () {
      const f = fixture(), contextRef = context(), facts: WriterSettlementFacts[] = [];
      const backend = { acquire: () => Effect.sync(() => {
        f.order.push("lease"); return { settle: (value: WriterSettlementFacts) => Effect.sync(() => { f.order.push("settle"); facts.push(value); }) };
      }) };
      const attempt = yield* f.run(withWriter(backend, contextRef, (permit) => Effect.gen(function* () {
        const allocation = yield* acquireRemote({ launch: { ...recipe, context: { reference: contextRef, persist: true } }, contextWriter: permit }, f.local);
        return allocation.attempt;
      }), { verify: () => Effect.sync(() => { f.order.push("readback"); }) }));
      assert.deepEqual(f.order, ["lease", "allocate", "fence", "capture", "initialization", "disconnect", "release", "status", "readback", "settle"]);
      ...
      assert.deepEqual(f.bodies[0], {
        projectId: account.projectId, timeout: 60, keepAlive: true, region: "us-east-1", proxies: false,
        browserSettings: { verified: true, recordSession: false, logSession: false, solveCaptchas: false, context: { id: contextRef.contextId, persist: true } },
  ...
  ...[401, 429].map((status) => ({
    ...
    run: Effect.gen(function* () {
      const f = fixture({ create: async () => Response.json({ error: "private upstream message" }, { status, headers: { "retry-after": "2" } }) });
      const result = yield* f.run(Effect.scoped(expectFailure(acquireRemote({ launch: recipe }, f.local))));
      assert.ok(result instanceof AllocationError);
      if (result instanceof AllocationError) {
        assert.equal(result.outcome, "rejected"); assert.equal(result.status, status); assert.equal(result.retryAfterMillis, 2000);
  ...
  })),
  {
    ...
    run: Effect.gen(function* () {
      const f = fixture({ create: async () => { throw new Error("network reply lost"); } }), facts: WriterSettlementFacts[] = [];
      yield* f.run(withWriter({ acquire: () => Effect.succeed({ settle: (value: WriterSettlementFacts) => Effect.sync(() => { facts.push(value); }) }) }, context(), (permit) =>
        expectFailure(acquireRemote({ launch: { ...recipe, context: { reference: permit.reference, persist: true } }, contextWriter: permit }, f.local)),
      ));
      assert.deepEqual(f.order, ["allocate"]);
  ...
  },
  {
    ...
    run: Effect.gen(function* () {
      let reads = 0;
      const f = fixture({ retrieve: async () => Response.json({ ...metadata(++reads >= 3 ? "COMPLETED" : "RUNNING"), connectUrl: "ws://localhost:9222" }) });
      const value = yield* f.run(Effect.scoped(Effect.gen(function* () {
  ...
  ...(["fence", "capture", "initialization", "disconnect"] as const).map((localFailure) => ({
    ...
    run: Effect.gen(function* () {
      ...
      const result = yield* f.run(Effect.scoped(Effect.gen(function* () {
        const allocation = yield* acquireRemote({ launch: recipe }, f.local);
        return yield* allocation.release;
      })));
      assert.deepEqual(f.order, ["allocate", "fence", "capture", "initialization", "disconnect", "release", "status"]);
      assert.deepEqual(result.issues.map((issue) => issue.step), [localFailure]);
  ...
  })),
  {
    ...
    run: Effect.gen(function* () {
      ...
      const result = yield* f.run(Effect.scoped(Effect.gen(function* () {
        const allocation = yield* acquireRemote({ launch: recipe }, f.local); return yield* allocation.release;
      })));
      assert.equal(result.releaseRequested, false); assert.equal(result.remote, "confirmed");
      assert.deepEqual(result.issues.map((issue) => issue.step), ["release"]);
  ...
  },
  {
    ...
    run: Effect.scoped(Effect.gen(function* () {
      const f = fixture(), entered = yield* Deferred.make<void>(), done = yield* Deferred.make<void>();
      yield* f.run(Effect.gen(function* () {
        const local = { ...f.local, capture: f.local.capture.pipe(Effect.andThen(Deferred.succeed(entered, undefined)), Effect.andThen(Deferred.await(done))) };
        ...
        const a = yield* Fiber.join(first), b = yield* Fiber.join(second);
        assert.equal(a, b); assert.equal(Object.isFrozen(a), true);
  ...
  },
  {
    ...
    run: Effect.gen(function* () {
      const f = fixture();
      const result = yield* f.run(Effect.gen(function* () {
        const coordinator = yield* makeCleanup(ref, "borrowed", yield* BrowserbaseSessions, f.local, () => {});
        return yield* coordinator.close;
      }));
      assert.equal(result.remote, "not-owned"); assert.equal(result.releaseRequested, false);
  ...
  },
  {
    ...
    run: Effect.gen(function* () {
      const f = fixture();
      yield* f.run(Effect.scoped(Effect.gen(function* () {
        const error = yield* expectFailure(acquireRemote({ launch: { ...recipe, context: { reference: context(), persist: true } } }));
        assert.equal(error.reason, "authorization");
        const invalid = yield* expectFailure(acquireRemote({ launch: { ...recipe, viewport: { _tag: "Fixed", width: 800, height: 600 } } }));
  ...
  },
  {
    ...
    run: Effect.scoped(Effect.gen(function* () {
      ...
      const f = fixture({ create: async (request) => {
        signalEntered();
        return new Promise<Response>((_resolve, reject) => {
          request.signal.addEventListener("abort", () => reject(new Error("create interrupted")), { once: true });
        });
      } });
      const running = f.run(withWriter({ acquire: () => Effect.succeed({ settle: (value: WriterSettlementFacts) => Effect.sync(() => { facts.push(value); }) }) }, context(), (permit) =>
        acquireRemote({ launch: { ...recipe, context: { reference: permit.reference, persist: true } }, contextWriter: permit }, f.local),
      ));
  ...
  },
  {
    ...
    run: Effect.gen(function* () {
      const f = fixture({ create: async () => Response.json(metadata("RUNNING", "foreign-project")) });
      const error = yield* f.run(Effect.scoped(expectFailure(acquireRemote({ launch: recipe }, f.local))));
  ...
  },
  {
    ...
    run: Effect.gen(function* () {
      ...
      assert.equal(result.remote, "pending"); assert.equal(result.releaseRequested, true);
      assert.equal(result.observedStatus, undefined);
      assert.deepEqual(result.issues.map((issue) => issue.step), ["status"]);
    }),
  },
  ...(["capture", "initialization", "disconnect"] as const).map((hung) => ({
    name: `${hung} deadline records timeout and still requests remote release`,
    run: testTime(Effect.gen(function* () {
      const f = fixture();
      ...
      assert.equal(result.remote, "confirmed");
      assert.deepEqual(result.issues.map((issue) => [issue.step, issue.reason]), [[hung, "timeout"]]);
  ...
  })),
  {
    ...
    run: Effect.scoped(Effect.gen(function* () {
      const f = fixture(), entered = yield* Deferred.make<void>(), done = yield* Deferred.make<void>();
      yield* f.run(Effect.gen(function* () {
        const local = { ...f.local, capture: Deferred.succeed(entered, undefined).pipe(Effect.andThen(Deferred.await(done))) };
        const allocation = yield* acquireRemote({ launch: recipe }, local);
