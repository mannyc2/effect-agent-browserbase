...
const account = {
  ...
  apiKey: Redacted.make("stream-key"),
...
const transfer = (media: boolean) => ({
  ...
  run: Effect.gen(function* () {
          ...
          request.signal.addEventListener(
            ...
            () => {
              ...
              if (!done) {
                ...
                controller.error(new Error("request ended before body"));
              }
            },
            { once: true },
          );
    ...
    yield* Effect.gen(function* () {
      ...
      assert.deepEqual(
        Array.from(chunks).flatMap((chunk) => [...chunk]),
        [1, 2, 3],
      );
      assert.equal(aborted, 1);
    }).pipe(
      Effect.provide(BrowserbaseClient.layer(account)),
      Effect.provideService(FetchHttpClient.Fetch, fetch),
    );
  }),
});

export const streamingCases = [
  transfer(false),
  transfer(true),
  {
