...
import { makeLocalCleanup } from "../src/internal/browser/LocalSession.ts";
import { LocalBrowser, LocalReference, type LocalCleanupResult } from "../src/LocalBrowser.ts";

const reference = LocalReference.make({ provider: "local", id: "one-local-lifetime" });
...
it.effect(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const cleanup = yield* makeLocalCleanup(
        reference,
        {
          fence: mark("fence"),
      ...
      Effect.gen(function* () {
        ...
        for (const endpoint of [
          ...
          const error = yield* Effect.gen(function* () {
            return yield* (yield* LocalBrowser).attach(Redacted.make(endpoint), {
        ...
        for (const arg of [
          ...
          const error = yield* Layer.build(LocalBrowser.layer({ launch: { args: [arg] } })).pipe(
            Effect.flip,
          );
