...
import { LocalBrowser, type LocalCleanupResult } from "effect-browserbase/local-browser";
      ...
      Effect.gen(function* () {
        ...
        yield* Effect.scoped(
          Effect.gen(function* () {
            ...
            expect(session.reference.provider).toBe("local");
  ...
  () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        yield* Effect.scoped(
          Effect.gen(function* () {
            const session = yield* (yield* LocalBrowser).attach(host.endpoint, { policy });
        ...
        const failed = yield* Effect.scoped(
          Effect.gen(function* () {
            return yield* (yield* LocalBrowser).attach(host.endpoint, {
              policy,
              target: { targetId: "does-not-exist" },
            });
          }),
        ).pipe(Effect.provide(LocalBrowser.layer()), Effect.result);
