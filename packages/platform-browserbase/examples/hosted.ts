...
import { RecordingPageReference, type SessionReference } from "@effect-agent/platform-browserbase/types";
...
export const inspectPage = (credentials: Credentials, url: string) =>
  transport(Effect.scoped(Effect.gen(function* () {
...
export const persistentReconnect = (
  credentials: Credentials,
  contextId: string,
  acquireExclusiveLease: (request: { readonly projectId: string; readonly contextId: string }) =>
  ...
  return transport(Effect.scoped(Effect.gen(function* () {
    const session = yield* (yield* BrowserbaseInteractiveHost).open(policy);
    yield* session.handle.navigate(BrowserNavigateRequest.make({ url }));
...
export const downloadFile = (credentials: Credentials, url: string) => {
  ...
  return transport(Effect.scoped(Effect.gen(function* () {
    ...
    const fresh = yield* downloads.waitForNew(session.reference, before.downloads.map((item) => item.id));
    const file = fresh[0];
    if (file === undefined) return yield* Effect.die("provider returned no new download");
    const bytes = yield* downloads.stream(session.reference, file.id, {
...
export const retrieveRecording = (
  ...
  return transport(Effect.gen(function* () {
    ...
    const bytes = yield* recordings.download(
      RecordingPageReference.make({ session: reference, pageId }),
