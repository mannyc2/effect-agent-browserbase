...
export class BrowserbaseDownloads extends Context.Service<
  BrowserbaseDownloads,
  {
    readonly list: (
      ...
      offset?: number,
    ) => Effect.Effect<DownloadListing, BrowserbaseError>;
    readonly metadata: (
      reference: SessionReference,
      downloadId: string,
    ) => Effect.Effect<DownloadMetadata, BrowserbaseError>;
    readonly stream: (
      reference: SessionReference,
      downloadId: string,
    ...
    ) => Effect.Effect<ReadonlyArray<DownloadMetadata>, BrowserbaseError>;
  }
>()("@effect-agent/platform-browserbase/BrowserbaseDownloads") {
  static layer(options: BrowserbaseOptions) {
    return Layer.effect(
      this,
      makeHttp(options).pipe(Effect.map((http) => makeDownloads(http, options.projectId))),
    );
  }
}
...
const makeDownloads = (http: Http, projectId: string): BrowserbaseDownloads["Service"] => {
  ...
  const list = Effect.fnUntraced(function* (ref: SessionReference, offset = 0) {
    if (!Number.isSafeInteger(offset) || offset < 0 || offset > 10_000)
      return yield* BrowserbaseError.make({ operation: "downloads-list", reason: "configuration" });
    ...
    const raw = yield* http
      ...
      .pipe(Effect.flatMap((value) => decode(Listing, value, "downloads-list")));

    if (
      raw.downloads.some((d) => d.sessionId !== ref.sessionId) ||
      new Set(raw.downloads.map((d) => d.id)).size !== raw.downloads.length
    ) {
  ...
  const metadata = Effect.fnUntraced(function* (ref: SessionReference, id: string) {
    ...
    const value = yield* http
      .json("GET", `/v1/downloads/${encodeURIComponent(id)}`)
      .pipe(Effect.flatMap((raw) => decode(DownloadMetadata, raw, "download-metadata")));

    if (value.id !== id || value.sessionId !== ref.sessionId)
      return yield* BrowserbaseError.make({ operation: "download-metadata", reason: "malformed" });
              ...
              Stream.fromEffect(
                Effect.suspend(() =>
                  received === file.size
                    ? Effect.void
                    : Effect.fail(
                        BrowserbaseError.make({ operation: "download", reason: "malformed" }),
                      ),
                ),
              ).pipe(Stream.drain),
            ),
  ...
  const waitForNew = Effect.fnUntraced(function* (
    ...
    do {
      ...
      if (!result.complete)
        return yield* BrowserbaseError.make({ operation: "download-wait", reason: "limit" });
