...
const makeDownloads = (http: Http, projectId: string): BrowserbaseDownloads["Service"] => {
  ...
  const list = Effect.fnUntraced(function* (ref: SessionReference, offset = 0) {
    ...
    if (raw.downloads.some((d) => d.sessionId !== ref.sessionId) || new Set(raw.downloads.map((d) => d.id)).size !== raw.downloads.length) {
      return yield* new BrowserbaseError({ operation: "downloads-list", reason: "malformed" });
    }
  ...
  const stream = (ref: SessionReference, id: string, policy: DownloadPolicy) => Stream.unwrap(Effect.gen(function* () {
    ...
    if (!policy.mimeTypes.includes(file.mimeType.toLowerCase())) return yield* new BrowserbaseError({ operation: "download", reason: "content-type" });
    if (file.size > policy.maxBytes) return yield* new BrowserbaseError({ operation: "download", reason: "limit" });
    let received = 0;
  ...
  const waitForNew = Effect.fnUntraced(function* (ref: SessionReference, previousIds: ReadonlyArray<string>, timeoutMillis = 30_000) {
    ...
    do {
      ...
      if (!result.complete) return yield* new BrowserbaseError({ operation: "download-wait", reason: "limit" });
    ...
    return yield* new BrowserbaseError({ operation: "download-wait", reason: "timeout" });
  });
