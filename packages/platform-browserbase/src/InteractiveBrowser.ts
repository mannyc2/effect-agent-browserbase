...
import {
  BrowserActionResult, BrowserClickRequest, BrowserFillRequest, BrowserNavigateRequest,
  BrowserNavigationResult, BrowserReadTextRequest, BrowserScreenshotRequest, BrowserScrollRequest,
  BrowserTextResult, InteractiveBrowser, InteractiveBrowserActionError, InteractiveBrowserBusyError,
  InteractiveBrowserExpiredError, InteractiveBrowserPolicy,
  InteractiveBrowserPolicyDeniedError, InteractiveBrowserProtocolError, InteractiveBrowserUnsupportedError,
  type BrowserHandle, type InteractiveBrowserError,
...
import { acquireSession, type BoundControls, type ContextLease, type SessionControls } from "./internal/Session.ts";
...
export const browserbaseInteractiveImplementation = SandboxImplementation.make({
  isolation: "isolated", identity: "browserbase-playwright-cdp",
});
...
export interface InteractiveOptions extends BrowserbaseOptions {
  ...
  readonly contextLease?: (request: { readonly projectId: string; readonly contextId: string }) => Effect.Effect<ContextLease, BrowserbaseError>;
...
export interface LiveView {
  ...
  readonly pages: ReadonlyArray<{ readonly liveViewPageId: string; readonly url: Redacted.Redacted<string> }>;
  readonly requestedTtlSeconds: number;
}
export interface Handoff { readonly token: Redacted.Redacted<string>; readonly view: LiveView }
export interface Resumed { readonly handle: BrowserHandle; readonly observation: Observation }
...
export interface BrowserbaseSession {
  ...
  readonly observe: (options?: { readonly maxTextBytes?: number; readonly maxControls?: number }) => Effect.Effect<Observation, BrowserbaseError>;
  readonly clickElement: (reference: ObservedElement) => Effect.Effect<BrowserActionResult, BrowserbaseError>;
  readonly fillElement: (reference: ObservedElement, value: string) => Effect.Effect<BrowserActionResult, BrowserbaseError>;
  ...
  readonly waitFor: (request: { readonly selector: string; readonly state: "visible" | "hidden" | "attached" | "detached" }) => Effect.Effect<void, BrowserbaseError>;
  /** Navigation observation is registered before the single click dispatch. */
  readonly clickAndWait: (request: BrowserClickRequest) => Effect.Effect<BrowserActionResult, BrowserbaseError>;
  readonly clickForDownload: (request: BrowserClickRequest) => Effect.Effect<DownloadObservation, BrowserbaseError>;
  ...
  readonly resume: (token: Redacted.Redacted<string>, operatorReleasedControl: boolean) => Effect.Effect<Resumed, BrowserbaseError>;
  readonly detach: Effect.Effect<{ readonly reference: SessionReference; readonly targetId: string }, BrowserbaseError>;
  readonly reconnect: (operatorReleasedControl: boolean) => Effect.Effect<Resumed, BrowserbaseError>;
...
const operationError = (operation: InteractiveBrowserActionError["operation"], error: BrowserbaseError): InteractiveBrowserError => {
  const implementation = browserbaseInteractiveImplementation;
  switch (error.reason) {
    case "busy": return InteractiveBrowserBusyError.make({ implementation, message: "Browser control is busy or handed to an operator" });
    case "closed": case "expired": case "disconnected": case "stale":
      return InteractiveBrowserExpiredError.make({ implementation, message: "This browser handle or target is no longer usable" });
    case "configuration": return InteractiveBrowserPolicyDeniedError.make({ implementation, message: "The browser request is malformed" });
    case "malformed": return InteractiveBrowserProtocolError.make({ implementation, message: "The browser returned an invalid bounded result" });
    default: return InteractiveBrowserActionError.make({ implementation, operation,
      message: error.outcome === "undispatched" ? "The browser action was not dispatched" :
        error.outcome === "rejected" ? "The browser action was rejected" : "The browser action failed; its outcome may be unknown" });
  }
};
...
const makeHandle = (bound: BoundControls, controls: SessionControls): BrowserHandle => ({
  navigate: (request) => checked(BrowserNavigateRequest, request, "navigate").pipe(
    Effect.flatMap((r) => bound.navigate(r.url)), Effect.flatMap((url) => decode(BrowserNavigationResult, { url }, "navigate")),
    Effect.mapError((error) => operationError("navigate", error))),
  readText: (request) => checked(BrowserReadTextRequest, request, "read-text").pipe(
    Effect.flatMap((r) => bound.readText(r.selector)), Effect.flatMap((text) => decode(BrowserTextResult, { text }, "read-text")),
    Effect.mapError((error) => operationError("read-text", error))),
  click: (request) => checked(BrowserClickRequest, request, "click").pipe(
    Effect.flatMap((r) => bound.click(r.selector)), Effect.flatMap(actionResult), Effect.mapError((error) => operationError("click", error))),
  fill: (request) => checked(BrowserFillRequest, request, "fill").pipe(
    Effect.flatMap((r) => bound.fill(r.selector, r.value)), Effect.flatMap(actionResult), Effect.mapError((error) => operationError("fill", error))),
  scroll: (request) => checked(BrowserScrollRequest, request, "scroll").pipe(
    Effect.flatMap((r) => bound.scroll(r.deltaX, r.deltaY)), Effect.flatMap(actionResult), Effect.mapError((error) => operationError("scroll", error))),
  screenshot: (request) => checked(BrowserScreenshotRequest, request, "screenshot").pipe(
    Effect.flatMap((r) => bound.screenshot(r.fullPage)), Effect.flatMap((bytes) => decode(PageScreenshotResult, {
...
const makeSession = (controls: SessionControls): BrowserbaseSession => {
  ...
  const session: BrowserbaseSession = {
    ...
    handle: makeHandle(controls.bind(), controls), currentHandle, target: controls.currentTarget,
    observe: (options = {}) => controls.observe(options.maxTextBytes, options.maxControls),
    clickElement: (reference) => checked(ObservedElement, reference, "click").pipe(
      Effect.flatMap((r) => controls.bind().click(r)), Effect.flatMap(actionResult)),
    fillElement: (reference, value) => checked(ObservedElement, reference, "fill").pipe(
    ...
    clickAndWait: (request) => checked(BrowserClickRequest, request, "click-and-wait").pipe(
      Effect.flatMap((r) => controls.clickAndWait(r.selector)), Effect.flatMap(actionResult)),
    clickForDownload: (request) => checked(BrowserClickRequest, request, "download-action").pipe(
...
export class BrowserbaseInteractiveHost extends Context.Service<BrowserbaseInteractiveHost, {
  readonly acquire: (policy: InteractiveBrowserPolicy) => Effect.Effect<BrowserbaseAcquisition, BrowserbaseError | InteractiveBrowserError, Scope.Scope>;
  readonly open: (policy: InteractiveBrowserPolicy) => Effect.Effect<BrowserbaseSession, BrowserbaseError | InteractiveBrowserError, Scope.Scope>;
  readonly reconcile: (reference: SessionReference) => Effect.Effect<CleanupResult, BrowserbaseError>;
  ...
  static layer(options: InteractiveOptions) {
    return Layer.effect(this, Effect.gen(function* () {
      ...
      const initialPage = options.initialPage === undefined ? undefined : yield* checked(Schema.Union([
        Schema.Struct({ targetId: Identifier }), Schema.Struct({ newPage: Schema.Literal(true) }),
      ]), options.initialPage, "configure");
      const driver = {
        viewport, maxPages, popupPolicy, dialogPolicy,
        ...(initialPage === undefined ? {} : "targetId" in initialPage ? { initialTargetId: initialPage.targetId } : { newPage: true }),
      };
      const recordSession = yield* checked(Schema.Boolean, options.recordSession ?? false, "configure");
      const keepAlive = yield* checked(Schema.Boolean, options.keepAlive ?? false, "configure");
      const settings = {
        viewport, driver, recordSession, keepAlive,
      ...
      const acquire = Effect.fnUntraced(function* (policy: InteractiveBrowserPolicy) {
        ...
        if (fixed.network._tag !== "Unrestricted") return yield* InteractiveBrowserUnsupportedError.make({
          ...
          message: fixed.network._tag === "PublicWeb" ? "Browserbase does not establish connection-time public-address containment" :
        ...
        return {
          reference: acquired.reference, attempt: acquired.attempt, close: acquired.close,
          connect: acquired.connect.pipe(Effect.andThen(connected)),
        };
      });
      return BrowserbaseInteractiveHost.of({ acquire,
...
export const browserbaseInteractiveLayer = (options: InteractiveOptions) => Layer.effect(InteractiveBrowser,
  Effect.gen(function* () {
