...
import { BrowserbaseError, Identifier, RecordingBatch, RecordingPage, RecordingPageReference, SessionReference } from "./Types.ts";
...
export interface PollOptions { readonly timeoutMillis?: number; readonly intervalMillis?: number }
export interface DownloadLimits { readonly maxBytes: number; readonly timeoutMillis?: number }

/** Independent of a live browser. Merely building this Layer performs no provider calls. */
export class BrowserbaseRecordings extends Context.Service<BrowserbaseRecordings, {
  readonly request: (reference: SessionReference, mode?: "initial" | "retry-failed") => Effect.Effect<RecordingBatch, BrowserbaseError>;
  readonly status: (reference: SessionReference) => Effect.Effect<RecordingBatch, BrowserbaseError>;
  readonly wait: (reference: SessionReference, options?: PollOptions) => Effect.Effect<RecordingBatch, BrowserbaseError>;
  readonly download: (reference: RecordingPageReference, limits: DownloadLimits) => Stream.Stream<Uint8Array, BrowserbaseError>;
}>()("@effect-agent/platform-browserbase/BrowserbaseRecordings") {
  static layer(options: BrowserbaseOptions) {
    return Layer.effect(this, makeHttp(options).pipe(Effect.flatMap((http) => makeRecordings(http, options.projectId))));
  }
}
...
const makeRecordings = Effect.fnUntraced(function* (http: Http, projectId: string) {
  ...
  const project = (ref: SessionReference, raw: typeof RawBatch.Type, timedOut = false) => RecordingBatch.make({
    ...
    pages: raw.downloads.map((page) => RecordingPage.make({
      pageId: page.pageId, status: page.status,
      delivery: page.status !== "COMPLETED" ? "not-ready" : page.downloadUrl === undefined ? "external-storage" : "download",
  ...
  const request = (ref: SessionReference, mode: "initial" | "retry-failed" = "initial") => requests.withPermits(1)(
    Effect.gen(function* () {
      ...
      const needed = mode === "retry-failed" ? before.downloads.some((p) => p.status === "FAILED") :
  ...
  const wait = Effect.fnUntraced(function* (ref: SessionReference, options: PollOptions = {}) {
    ...
    for (;;) {
      const result = yield* within(read(ref, deadline), deadline, "recording-wait").pipe(Effect.result);
      if (result._tag === "Success") {
        latest = result.success;
        if (latest.downloads.every((p) => p.status === "COMPLETED" || p.status === "FAILED")) return project(ref, latest);
      } else if (!["timeout", "rate-limited", "provider", "transport"].includes(result.failure.reason)) {
  ...
  const download = (ref: RecordingPageReference, limits: DownloadLimits) => Stream.unwrap(Effect.gen(function* () {
