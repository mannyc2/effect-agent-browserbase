...
const playlistUrls = (body: string, allowed: (url: string) => boolean): ReadonlyArray<string> => {
  ...
  if (lines[0] !== "#EXTM3U" || !lines.includes("#EXT-X-ENDLIST") || lines.length > 32768) {
    throw new BrowserbaseError({ operation: "replay-playlist", reason: "malformed" });
  }
  const urls: string[] = [];
  for (const raw of lines.slice(1)) {
    ...
    if (line.startsWith("#EXT-X-STREAM-INF") || line.startsWith("#EXT-X-MEDIA:") ||
      ...
      throw new BrowserbaseError({ operation: "replay-playlist", reason: "unsupported" });
    }
    let url: string | undefined;
    if (!line.startsWith("#")) url = line;
    else if (/URI\s*=/.test(line)) {
      if (!line.startsWith("#EXT-X-MAP:")) throw new BrowserbaseError({ operation: "replay-playlist", reason: "unsupported" });
      const match = /(?:^|,)URI="([^"]+)"(?:,|$)/.exec(line.slice("#EXT-X-MAP:".length));
      if (match === null) throw new BrowserbaseError({ operation: "replay-playlist", reason: "malformed" });
      url = match[1];
    }
    if (url !== undefined) {
      if (!allowed(url) || urls.length >= 16384) throw new BrowserbaseError({ operation: "replay-playlist", reason: "unsafe-url" });
      urls.push(url);
    }
  }
  if (urls.length === 0) throw new BrowserbaseError({ operation: "replay-playlist", reason: "malformed" });
  return urls;
};
...
const makeReplays = (http: Http, projectId: string): BrowserbaseReplays["Service"] => {
  ...
  const metadata = Effect.fnUntraced(function* (ref: SessionReference) {
    ...
    if (!terminal(session.status)) return yield* new BrowserbaseError({ operation: "replay", reason: "active" });
    ...
    if (raw.pageCount !== raw.pages.length || new Set(raw.pages.map((p) => p.pageId)).size !== raw.pages.length ||
      ...
      return yield* new BrowserbaseError({ operation: "replay-metadata", reason: "malformed" });
    }
  ...
  const openPage = Effect.fnUntraced(function* (ref: RecordingPageReference) {
    ...
    if (!pages.some((p) => p.pageId === ref.pageId)) return yield* new BrowserbaseError({ operation: "replay", reason: "not-found" });
    ...
    const urls = yield* Effect.try({
      ...
      catch: () => new BrowserbaseError({ operation: "replay-playlist", reason: "malformed" }),
    });
    return {
      reference: ref,
      ...
      media: (index: number, limits: DownloadLimits) => {
        ...
        return url === undefined ? Stream.fail(new BrowserbaseError({ operation: "replay-media", reason: "not-found" })) :
