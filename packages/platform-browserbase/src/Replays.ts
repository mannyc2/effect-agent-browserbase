...
export class BrowserbaseReplays extends Context.Service<
  BrowserbaseReplays,
  {
    ...
    ) => Effect.Effect<ReadonlyArray<ReplayPage>, BrowserbaseError>;
    readonly openPage: (
      reference: RecordingPageReference,
    ) => Effect.Effect<ReplayAccess, BrowserbaseError>;
  }
>()("@effect-agent/platform-browserbase/BrowserbaseReplays") {
  static layer(options: BrowserbaseOptions) {
    return Layer.effect(
      this,
      makeHttp(options).pipe(Effect.map((http) => makeReplays(http, options.projectId))),
    );
  }
}
...
const playlistUrls = (body: string, allowed: (url: string) => boolean): ReadonlyArray<string> => {
  ...
  for (const raw of lines.slice(1)) {
    ...
    if (url !== undefined) {
      if (!allowed(url) || urls.length >= 16384)
        throw BrowserbaseError.make({ operation: "replay-playlist", reason: "unsafe-url" });
      urls.push(url);
    }
  }
  if (urls.length === 0)
...
const makeReplays = (http: Http, projectId: string): BrowserbaseReplays["Service"] => {
  ...
  const metadata = Effect.fnUntraced(function* (ref: SessionReference) {
    ...
    if (!terminal(session.status))
      return yield* BrowserbaseError.make({ operation: "replay", reason: "active" });

    const raw = yield* http
      .json("GET", `/v1/sessions/${encodeURIComponent(ref.sessionId)}/replays`)
      .pipe(Effect.flatMap((value) => decode(Metadata, value, "replay-metadata")));

    if (
      raw.pageCount !== raw.pages.length ||
      new Set(raw.pages.map((p) => p.pageId)).size !== raw.pages.length ||
    ...
    return raw.pages.map((p) =>
      ReplayPage.make({ pageId: p.pageId, startTimeMs: p.startTimeMs, endTimeMs: p.endTimeMs }),
    );
  });
  ...
  const openPage = Effect.fnUntraced(function* (ref: RecordingPageReference) {
    ...
    if (!pages.some((p) => p.pageId === ref.pageId))
      return yield* BrowserbaseError.make({ operation: "replay", reason: "not-found" });

    const body = yield* http.text(
