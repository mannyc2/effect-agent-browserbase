...
export const startCapture = Effect.fnUntraced(function* (parent: CaptureParent, options: CaptureOptions = {}) {
  ...
  yield* Effect.addFinalizer(() => Queue.shutdown(wake).pipe(Effect.andThen(Queue.shutdown(finished)), Effect.asVoid));
  ...
  let ended = false, subscribed = false, cleanupFinished = false;
  let reason = "stopped";
  let error: BrowserbaseError | undefined;
  let received = 0, delivered = 0, rejected = 0, duplicates = 0;
  let first: number | undefined, last: number | undefined;
  let geometry: { width: number; height: number; viewportWidth: number; viewportHeight: number } | undefined;
  ...
  const finish = (why: string, failure?: BrowserbaseError) => {
    if (ended) return;
    ended = true; reason = why; error = failure;
    Queue.offerUnsafe(wake, undefined); Queue.offerUnsafe(finished, undefined);
  };
  const snapshot = (): CaptureSummary => {
    ...
    return CaptureSummary.make({
      target, reason, received, delivered, dropped: buffer.dropped + rejected, duplicates,
      peakBufferedFrames: buffer.highWaterFrames, peakBufferedBytes: buffer.highWaterBytes,
      bufferedFrames: buffer.size, bufferedBytes: buffer.bytes,
      sourceFirstMillis: first ?? null, sourceLastMillis: last ?? null, nativeStop, upstreamDrops: "unknown",
      ...(error === undefined ? {} : { error }),
    });
  };

  const performStop = yield* Effect.cached(Effect.uninterruptibleMask((restore) => Effect.gen(function* () {
    finish("stopped");
    ...
    if (nativeStop === "confirmed" && parent.captureLease === lease) parent.captureLease = undefined;
  ...
  const receive = (frame: NativeFrame): void => {
    ...
    try {
      ...
      buffer.offer({
        bytes: new Uint8Array(frame.data), mediaType: "image/jpeg", target, sequence,
        sourceTimeMillis: meta.timestamp, sourceClock: "presentation-unix-millis",
        receivedMonotonicNanos: clock.monotonicTimeNanosUnsafe(),
        ...dimensions, viewportWidth: meta.viewportWidth, viewportHeight: meta.viewportHeight,
      });
  ...
  yield* parent.owner.guard("capture-start", (ticket) => Effect.gen(function* () {
    ...
    yield* Effect.addFinalizer(() => stopNative.pipe(Effect.andThen(Effect.sync(() => {
      rejected += buffer.size; buffer.clear();
    }))));
    startSettled = false;
    yield* Effect.tryPromise({
      try: () => {
        ...
        void startPromise.then(() => {
          startSettled = true;
          if (cleanupFinished && nativeStop === "unconfirmed" && parent.captureLease === lease) {
  ...
  const next: Effect.Effect<CapturedFrame, BrowserbaseError | Cause.Done> = Effect.suspend(() => {
    ...
    if (frame !== undefined) { delivered++; return Effect.succeed(frame); }
  ...
  return { frames, stop: stopNative, completed: Deferred.await(completed).pipe(Effect.map(snapshot)) } satisfies CaptureInterval;
});
