...
const Options = Schema.Struct({
  ...
  artifactOrigins: Schema.optionalKey(
    Schema.Array(Schema.String.check(Schema.isMaxLength(8192))).check(Schema.isMaxLength(32)),
  ),
...
const collect = (
  stream: Stream.Stream<Uint8Array, BrowserbaseError>,
  maximum: number,
  operation: string,
) =>
  Effect.gen(function* () {
    ...
    yield* Stream.runForEach(stream, (chunk) =>
      Effect.suspend(() => {
        if (chunk.byteLength > maximum - total)
          return Effect.fail(BrowserbaseError.make({ operation, reason: "limit" }));
...
export const makeHttp = Effect.fnUntraced(function* (
  ...
  const configured = yield* Schema.decodeEffect(Options)(options, {
    ...
    Effect.mapError(() =>
      BrowserbaseError.make({ operation: "configure", reason: "configuration" }),
    ),
  );
  ...
  const jsonOnce = Effect.fnUntraced(
    function* (method: "GET" | "POST", path: string, body?: Schema.Json) {
  ...
  const json = Effect.fnUntraced(function* (
    ...
    for (let attempt = 0; ; attempt++) {
      const result = yield* within(jsonOnce(method, path, body), deadline, "provider-request").pipe(
        Effect.result,
      );
  ...
  const media = (
    ...
    Stream.unwrap(
      Effect.gen(function* () {
        if (
          !validateMediaUrl(Redacted.value(url)) ||
          !Number.isSafeInteger(maximum) ||
          maximum < 1 ||
          maximum > 2 ** 31 - 1 ||
          !Number.isSafeInteger(timeoutMillis) ||
        ...
        const response = yield* within(
          execute(HttpClientRequest.get(Redacted.value(url)), "media-download"),
          deadline,
          "media-download",
        );
