...
export const makeOwner = Effect.fnUntraced(function* (limits: Limits) {
  ...
  const guard = <A, R>(
    ...
    options: {
      ...
      readonly preflight?: Effect.Effect<void, BrowserbaseError>;
    } = {},
  ): Effect.Effect<A, BrowserbaseError, R> =>
    semaphore
      ...
      .pipe(
        Effect.flatMap(
          Effect.fromOption(() =>
            BrowserbaseError.make({ operation, reason: "busy", outcome: "undispatched" }),
          ),
        ),
      );
