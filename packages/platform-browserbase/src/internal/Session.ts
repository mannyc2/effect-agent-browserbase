...
import { AllocationAttempt, BrowserbaseError, CleanupResult, Observation, Target, type ObservedElement, type SessionReference, type Viewport } from "../Types.ts";
...
export interface ContextLease {
  ...
  readonly finalize: (outcome: { readonly attempt: AllocationAttempt; readonly cleanup: Option.Option<CleanupResult> }) => Effect.Effect<void>;
}
...
export type Connector = (connection: unknown, signal: AbortSignal, options: DriverOptions, events: DriverEvents) => Promise<Driver>;
...
export const acquireSession = Effect.fnUntraced(function* (
  ...
  const terminate = yield* Effect.cached(Effect.uninterruptibleMask((restore) => Effect.gen(function* () {
    ...
    if (remote !== undefined) {
      ...
      if (options.onCleanup !== undefined) {
        ...
        cleanup = CleanupResult.make({ ...cleanup, reporting: Exit.isSuccess(report) ? "reported" : "failed" });
      }
    }
  })));

  const closeScope = yield* Effect.cached(Scope.close(resourceScope, Exit.void).pipe(Effect.ensuring(Deferred.succeed(ended, undefined))));
  ...
  const allocationUnknown = Effect.suspend(() => {
    ...
    return options.onAllocationUncertain === undefined ? Effect.void :
      boundedCleanup(options.onAllocationUncertain(attempt), "allocation-report").pipe(Effect.asVoid);
  });
  const created = yield* within(provider.create(attempt, options, (known) => {
  ...
  const connectNative = (url: Redacted.Redacted<unknown>, nativeOptions: DriverOptions) =>
    Effect.uninterruptibleMask((restore) => Effect.gen(function* () {
      ...
      const acquired = yield* restore(Effect.tryPromise({
        try: (signal) => {
  ...
  const nativeOperation = <A>(operation: string, action: (driver: Driver, ticket: Ticket) => Promise<A>,
    ...
    (ticket) => native(operation, ticket, () => action(getDriver(), ticket)), { mutation, charge });
  ...
  const bind = () => {
    ...
    const run = <A>(operation: string, action: (driver: Driver, ticket: Ticket) => Promise<A>, mutation = false) =>
      owner.guard(operation, (ticket) => native(operation, ticket, () => action(getDriver(), ticket)), { mutation, preflight: Effect.suspend(check) });
    return {
      navigate: (url: string) => run("navigate", (driver, ticket) => driver.navigate(url, ticket), true),
      readText: (selector?: string) => run("read-text", (driver, ticket) => driver.readText(selector, options.maxReturnedBytes, ticket)),
      click: (target: string | ObservedElement) => run("click", (driver, ticket) => driver.click(target, ticket), true),
      fill: (target: string | ObservedElement, value: string) => run("fill", (driver, ticket) => driver.fill(target, value, ticket), true),
      scroll: (x: number, y: number) => run("scroll", (driver, ticket) => driver.scroll(x, y, ticket), true),
      screenshot: (full: boolean) => run("screenshot", (driver, ticket) => driver.screenshot(full, options.maxReturnedBytes, ticket)),
    };
  };

  const connected = yield* Effect.cached(owner.guard("connect", (ticket) =>
    connectNative(created.connection, options.driver).pipe(
      Effect.andThen(native("connect", ticket, async () => { getDriver().selected(); })),
      Effect.tap(() => Effect.sync(() => { owner.state.phase = "open"; })),
    ), { phases: ["acquiring"], charge: false, verifyAfter: false }).pipe(
  ...
  const controls = {
    ...
    resize: (viewport: Viewport) => nativeOperation("resize", (driver, ticket) => driver.resize(viewport, ticket), true, false),
    ...
    clickAndWait: (target: string | ObservedElement) => nativeOperation("click-and-wait", (driver, ticket) => driver.clickAndWait(target, ticket), true),
    clickForDownload: (target: string | ObservedElement) => nativeOperation("download-action", (driver, ticket) => driver.clickForDownload(target, ticket), true),
    liveView: (ttl: number) => owner.guard("live-view", () => provider.liveView(ref, ttl), { charge: false, phases: ["open", "paused"] }),
    beginHandoff: (ttl: number) => owner.guard("handoff", () => Effect.gen(function* () {
      if (owner.state.phase === "open") owner.fence("paused", "paused");
...
export type SessionControls = Effect.Success<ReturnType<typeof acquireSession>> extends { connect: Effect.Effect<infer A, infer _E, infer _R> } ? A : never;
