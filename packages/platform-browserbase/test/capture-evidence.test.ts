import type { CapturedFrame } from "@effect-agent/platform-browserbase/capture";
import { CaptureSummary } from "@effect-agent/platform-browserbase/capture";
import { BrowserbaseError, Target } from "@effect-agent/platform-browserbase/types";
...
const summary = (delivered: number, received = delivered, duplicates = 0) =>
  CaptureSummary.make({
    ...
    error: BrowserbaseError.make({ operation: "PRIVATE-ERROR", reason: "provider" }),
  });
