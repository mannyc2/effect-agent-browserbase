...
const options = { projectId: "project-1", apiKey: Redacted.make("PRIVATE-API-KEY"), artifactOrigins: ["https://media.example.test"] };
const metadata = () => Response.json({ id: ref.sessionId, projectId: ref.projectId, status: "COMPLETED" });
const pending = { downloads: [{ pageId: "0", status: "PENDING" }] };
const completed = { downloads: [{ pageId: "0", status: "COMPLETED", downloadUrl: "https://media.example.test/video?token=PRIVATE-SIGNED" }] };
...
export const artifactCases = [
  { name: "recording request inspects status, posts once, and preserves per-page states", run: Effect.gen(function* () {
    ...
    yield* useRecordings(async (input, init) => {
      ...
      return Response.json(posts === 0 ? { downloads: [{ pageId: "0", status: "NOT_REQUESTED" }] } : pending);
  ...
  { name: "lost assembly response remains uncertain and is not automatically posted again", run: Effect.gen(function* () {
    ...
    yield* useRecordings(async (input, init) => {
      ...
      return Response.json(posts ? pending : { downloads: [{ pageId: "0", status: "NOT_REQUESTED" }] });
    }, (api) => Effect.gen(function* () {
  ...
  { name: "artifact download refreshes access, strips API credentials, and streams owned bytes", run: Effect.gen(function* () {
    ...
    yield* useRecordings(async (input, init) => {
      ...
      if (req.url.includes("/recording/")) return Response.json({ downloads: [{ pageId: "0", status: "COMPLETED", downloadUrl: `https://media.example.test/video?token=${++statusReads > 1 ? "fresh" : "old"}` }] });
      return metadata();
    }, (api) => Effect.gen(function* () {
      const status = yield* api.status(ref); assert.ok(!JSON.stringify(status).includes("downloadUrl"));
      const chunks = yield* Stream.runCollect(api.download(page, { maxBytes: 1000 }));
      assert.deepEqual([...chunks[0]!], [1, 2, 3]); chunks[0]![0] = 100;
      assert.equal(first[0], 1);
    }));
    assert.equal(statusReads, 2); assert.equal(mediaReads, 1);
  }) },
  ...
  { name: "credential-preserving API policy ignores ambient headers and redirect options", run: Effect.gen(function* () {
    let requests = 0;
    const http = yield* makeHttp(options, async (input, init) => {
      const request = new Request(input, init); requests++;
      assert.equal(request.redirect, "manual"); assert.equal(request.headers.has("authorization"), false);
      assert.equal(request.headers.has("x-ambient"), false); assert.equal(request.headers.has("traceparent"), false);
      assert.equal(request.headers.get("x-bb-api-key"), "PRIVATE-API-KEY");
      return Response.json({ ok: true });
    });
    yield* http.json("GET", "/v1/sessions/session-1").pipe(Effect.provideService(FetchHttpClient.RequestInit, {
      redirect: "follow", headers: { authorization: "PRIVATE-AMBIENT", "x-ambient": "wrong" },
    }));
  ...
  { name: "HTTP byte limits apply to actual streamed bytes without Content-Length", run: Effect.gen(function* () {
    const http = yield* makeHttp(options, async () => new Response(new ReadableStream({ start(controller) {
      controller.enqueue(new Uint8Array(8)); controller.enqueue(new Uint8Array(8)); controller.close();
    } }), { headers: { "content-type": "video/mp4" } }));
    yield* expectReason(Stream.runDrain(http.media(Redacted.make("https://media.example.test/video"), 10, ["video/mp4"])), "limit");
  }) },
  { name: "early stream completion releases the request scope", run: Effect.gen(function* () {
    let signal: AbortSignal | undefined;
    const http = yield* makeHttp(options, async (_input, init) => {
      signal = init?.signal ?? undefined;
      return new Response(new ReadableStream({ start(controller) { controller.enqueue(new Uint8Array([1])); } }), { headers: { "content-type": "video/mp4" } });
    });
    yield* http.media(Redacted.make("https://media.example.test/video"), 10, ["video/mp4"]).pipe(Stream.take(1), Stream.runDrain);
    assert.equal(signal?.aborted, true);
  }) },
  { name: "redirects do not forward provider credentials or signed-media access", run: Effect.gen(function* () {
    let calls = 0;
    const http = yield* makeHttp(options, async () => { calls++; return new Response(null, { status: 302, headers: { location: "https://attacker.invalid/" } }); });
    yield* expectReason(http.json("GET", "/v1/sessions/session-1"), "provider");
    yield* expectReason(Stream.runDrain(http.media(Redacted.make("https://media.example.test/video"), 10, ["video/mp4"])), "provider");
    assert.equal(calls, 2);
  }) },
  { name: "malformed UTF-8 is a sanitized protocol failure", run: Effect.gen(function* () {
    const http = yield* makeHttp(options, async () => new Response(new Uint8Array([0xC3, 0x28]), { headers: { "content-type": "application/json" } }));
    const result = yield* http.json("GET", "/v1/sessions/session-1").pipe(Effect.result);
  ...
  { name: "replay access serves existing VOD material without recording or browser allocation", run: Effect.gen(function* () {
    ...
    yield* useReplays(async (input, init) => {
      ...
      if (req.url.startsWith("https://media.example.test")) { assert.equal(req.headers.has("x-bb-api-key"), false); return new Response(new Uint8Array([1]), { headers: { "content-type": "video/mp4" } }); }
      return metadata();
    }, (api) => Effect.gen(function* () {
      const access = yield* api.openPage(page); assert.equal(access.mediaCount, 2);
  ...
  { name: "ordinary download metadata and streaming are separate from recording artifacts", run: Effect.gen(function* () {
    ...
    yield* useDownloads(async (input, init) => {
      ...
      const fresh = yield* api.waitForNew(ref, []); assert.equal(fresh[0]?.id, "download-1");
      const chunks = yield* Stream.runCollect(api.stream(ref, "download-1", { maxBytes: 100, mimeTypes: ["text/plain"] }));
      assert.equal(chunks.reduce((sum, bytes) => sum + bytes.length, 0), 3);
    }));
  }) },
  { name: "unsafe website filenames fail before a content request", run: Effect.gen(function* () {
    ...
    yield* useDownloads(async (input, init) => {
      ...
      if (req.url.includes("/v1/downloads/")) {
        ...
        return Response.json({ id: "download-1", sessionId: "session-1", filename: "../escape.txt", mimeType: "text/plain", size: 1, checksum: "0".repeat(64), createdAt: "2026-09-17T00:00:00Z" });
      }
      return metadata();
