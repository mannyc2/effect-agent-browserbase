...
const metadata = () =>
  Response.json({ id: ref.sessionId, projectId: ref.projectId, status: "COMPLETED" });
...
const withRecordings = <A>(
  ...
  body: (api: BrowserbaseRecordings["Service"]) => Effect.Effect<A, BrowserbaseError>,
) =>
  Effect.gen(function* () {
    return yield* body(yield* BrowserbaseRecordings);
  }).pipe(
    Effect.provide(BrowserbaseRecordings.layer(options)),
    Effect.provideService(FetchHttpClient.Fetch, fetch),
  );

const withReplays = <A>(
  fetch: typeof globalThis.fetch,
  body: (api: BrowserbaseReplays["Service"]) => Effect.Effect<A, BrowserbaseError>,
) =>
  Effect.gen(function* () {
    return yield* body(yield* BrowserbaseReplays);
  }).pipe(
    Effect.provide(BrowserbaseReplays.layer(options)),
    Effect.provideService(FetchHttpClient.Fetch, fetch),
  );

const withDownloads = <A>(
  fetch: typeof globalThis.fetch,
  body: (api: BrowserbaseDownloads["Service"]) => Effect.Effect<A, BrowserbaseError>,
) =>
  Effect.gen(function* () {
    return yield* body(yield* BrowserbaseDownloads);
  }).pipe(
    Effect.provide(BrowserbaseDownloads.layer(options)),
    Effect.provideService(FetchHttpClient.Fetch, fetch),
  );
...
export const artifactCases = [
  ...
  },
  {
    ...
    run: Effect.gen(function* () {
      ...
      yield* http.json("GET", "/v1/sessions/session-1").pipe(
  ...
  },
  {
    ...
    run: Effect.gen(function* () {
      ...
      const http = yield* makeHttp(options, async () => {
        calls++;

        return calls < 3 ? Response.json({}, { status: 500 }) : Response.json({ ok: true });
      });

      yield* elapse(http.json("GET", "/v1/sessions/session-1"), 450);
      assert.equal(calls, 3);
      calls = 0;
      yield* expectReason(http.json("POST", "/v1/sessions", {}), "provider");
  ...
  },
  {
    ...
    run: Effect.gen(function* () {
      ...
      yield* expectReason(
        Stream.runDrain(
          http.media(Redacted.make("https://media.example.test/video"), 10, ["video/mp4"]),
        ),
        "limit",
      );
    }),
