...
import { BrowserbaseError, Target } from "../../src/Types.ts";
...
const makeFixture = Effect.fnUntraced(function* (options: {
  ...
  const owner = yield* makeOwner({ maxActions: 20, maxElapsedMillis: 30_000, actionTimeoutMillis: 10_000 });
  ...
  const parent: CaptureParent = {
    owner,
    target: () => Target.make({ generation: owner.state.generation, pageId: "page-1", frameId: "frame-1" }),
    source: () => Effect.succeed({
      ...
      stop: async () => { stops++; await options.stop?.(); },
    }),
  };
...
export const captureCases: ReadonlyArray<Case> = [
  test("capture has a bounded drop-oldest queue with explicit sequence gaps", () => Effect.gen(function* () {
    ...
    assert.deepEqual(frames.map((frame) => frame.sequence), [98, 99]);
    assert.equal((yield* interval.completed).delivered, 2);
    assert.equal(f.counts().stops, 1);
  })),
  test("byte pressure is enforced independently of the frame count", () => Effect.gen(function* () {
    const f = yield* makeFixture();
    const size = jpeg().byteLength;
    const interval = yield* startCapture(f.parent, { maxFrames: 10, maxBufferedBytes: size * 2, maxFrameBytes: size });
  ...
  test("capture copies callback bytes and preserves source time separately from receipt time", () => Effect.gen(function* () {
    const f = yield* makeFixture();
    const interval = yield* startCapture(f.parent, options);
    const data = jpeg();
  ...
  test("duplicate timestamps are accounted for, not silently relabeled as new frames", () => Effect.gen(function* () {
    const f = yield* makeFixture();
    const interval = yield* startCapture(f.parent, options);
    f.emit(1000); f.emit(1000); f.emit(1010);
    const summary = yield* interval.stop;
    const frames = yield* Stream.runCollect(interval.frames);
    assert.equal(summary.duplicates, 1);
    assert.equal(summary.dropped, 1);
    assert.deepEqual(frames.map((frame) => frame.sequence), [0, 2]);
  })),
  test("a backwards source timestamp ends with a typed partial stream", () => Effect.gen(function* () {
    const f = yield* makeFixture();
    const interval = yield* startCapture(f.parent, options);
    f.emit(1000); f.emit(900);
    const delivered: number[] = [];
    yield* expectReason(Stream.runForEach(interval.frames, (frame) => Effect.sync(() => { delivered.push(frame.sequence); })), "timestamp");
  ...
  test("actual JPEG geometry changes terminate an interval even at unchanged CSS dimensions", () => Effect.gen(function* () {
    const f = yield* makeFixture();
    const interval = yield* startCapture(f.parent, options);
    f.emit(1000); f.emit(1010, widerJpeg());
    yield* expectReason(Stream.runDrain(interval.frames), "resized");
    assert.equal((yield* interval.completed).received, 2);
  })),
  test("CSS viewport changes segment capture even when image dimensions are unchanged", () => Effect.gen(function* () {
    const f = yield* makeFixture();
    const interval = yield* startCapture(f.parent, options);
    f.emit(1000); f.emit(1010, jpeg(), 100, 48);
    yield* expectReason(Stream.runDrain(interval.frames), "resized");
  })),
  test("a malformed or oversized frame is rejected before retention", () => Effect.gen(function* () {
    for (const [data, reason] of [[new Uint8Array([1, 2, 3]), "malformed"], [new Uint8Array(9000), "limit"]] as const) {
      const f = yield* makeFixture();
      const interval = yield* startCapture(f.parent, options);
      f.emit(1000, data);
  ...
  test("stopping one interval permits another and a stale stop cannot end the replacement", () => Effect.gen(function* () {
    const f = yield* makeFixture();
  ...
  test("concurrent stop callers await one native stop rather than returning early", () => Effect.gen(function* () {
    const entered = gate<void>();
    const release = gate<void>();
    const f = yield* makeFixture({ stop: async () => { entered.resolve(); await release.promise; } });
    const interval = yield* startCapture(f.parent, options);
  ...
  test("a slow native stop quarantines the lease until confirmation", () => Effect.gen(function* () {
    const release = gate<void>();
    const entered = gate<void>();
    const f = yield* makeFixture({ stop: async () => { entered.resolve(); await release.promise; } });
    const interval = yield* startCapture(f.parent, options);
    ...
    release.resolve(); yield* Fiber.join(stopping);
    assert.equal(f.parent.captureLease, undefined);
  })),
  test("native stop failure prevents a second competing screencast without closing the browser", () => Effect.gen(function* () {
    const f = yield* makeFixture({ stop: async () => { throw new Error("PRIVATE-STOP"); } });
    const interval = yield* startCapture(f.parent, options);
  ...
  test("consumer cancellation ends capture but does not spend or close browser actions", () => Effect.gen(function* () {
    const f = yield* makeFixture();
    const interval = yield* startCapture(f.parent, options);
  ...
  test("a capture has one consumer and rejects a second without stopping the first", () => Effect.gen(function* () {
    const f = yield* makeFixture();
    const interval = yield* startCapture(f.parent, options);
    const consumer = yield* Stream.runDrain(interval.frames).pipe(Effect.forkChild);
    yield* Effect.yieldNow;
    ...
    yield* interval.stop; yield* Fiber.join(consumer);
  })),
  test("scope closure stops capture even when nobody subscribes", () => Effect.gen(function* () {
    const f = yield* makeFixture();
  ...
  test("parent generation fencing stops callbacks even when native disconnect fails", () => Effect.gen(function* () {
    let receive: ((frame: NativeFrame) => void) | undefined;
    let stops = 0;
    const f = yield* sessionFixture({ disconnectFails: true, captureSource: {
  ...
  test("ordinary page mutation leaves capture running but target changes segment it", () => Effect.gen(function* () {
    const f = yield* makeFixture();
    const interval = yield* startCapture(f.parent, options);
  ...
  test("failed native start runs cleanup and leaves no active capture lease", () => Effect.gen(function* () {
  ...
  test("interruption during native start observes its late result and cleans the lease", () => Effect.gen(function* () {
    const entered = gate<void>();
    const complete = gate<void>();
    const f = yield* makeFixture({ start: async () => { entered.resolve(); await complete.promise; } });
