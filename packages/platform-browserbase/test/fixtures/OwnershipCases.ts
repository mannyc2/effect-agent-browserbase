...
export const ownershipCases: ReadonlyArray<Case> = [
  test("one scoped session spans successive operations and closes once", () => Effect.gen(function* () {
    ...
    assert.equal(report.remote, "confirmed"); assert.equal(report.local, "closed");
    yield* session.close;
    assert.equal(f.state.allocations, 1); assert.equal(f.state.releases, 1); assert.equal(f.state.localCloses, 1);
  })),
  test("failed connection retains exact identity and still releases", () => Effect.gen(function* () {
    ...
    assert.equal(f.state.releases, 1); assert.equal(f.state.connects, 1);
    yield* expectReason(acquired.connect, "closed");
    assert.equal(f.state.connects, 1);
  })),
  test("lost create reply is not retried and reports an allocation nonce", () => Effect.gen(function* () {
    const f = yield* fixture({ createFails: true });
    yield* expectReason(f.acquisition, "allocation-unknown");
    assert.equal(f.state.allocations, 1); assert.equal(f.state.releases, 0); assert.equal(f.uncertain.length, 1);
  })),
  test("malformed allocation does not fabricate a session identity", () => Effect.gen(function* () {
    const f = yield* fixture({ malformedCreate: true });
    yield* expectReason(f.acquisition, "allocation-unknown");
    assert.equal(f.state.connects, 0); assert.equal(f.state.releases, 0); assert.equal(f.uncertain.length, 1);
  })),
  test("pending release needs an exact-session terminal read", () => Effect.gen(function* () {
    ...
    assert.ok(f.calls.includes("GET /v1/sessions/session-1")); assert.equal(f.state.releases, 1);
  })),
  ...
  test("interruption after dispatch expires automation without replay", () => Effect.gen(function* () {
    const entered = gate<void>(), done = gate<void>(); let dispatches = 0;
    const f = yield* fixture({ onClick: async (ticket) => { ticket.dispatch(); dispatches++; entered.resolve(); await done.promise; return "https://example.test/"; } });
    const session = yield* (yield* f.acquisition).connect;
    const fiber = yield* session.bind().click("#button").pipe(Effect.forkChild);
    yield* Effect.promise(() => entered.promise); yield* Fiber.interrupt(fiber);
    yield* expectReason(session.bind().readText(), "closed");
    done.resolve(); assert.equal(dispatches, 1);
  })),
  test("close wakes an in-flight native waiter and coalesces teardown", () => Effect.gen(function* () {
    const entered = gate<void>();
    const f = yield* fixture({ onClick: async (ticket) => { ticket.dispatch(); entered.resolve(); await new Promise(() => {}); return "https://example.test/"; } });
    const session = yield* (yield* f.acquisition).connect;
  ...
  test("handoff pauses automation and fresh observation commits under one permit", () => Effect.gen(function* () {
    const entered = gate<void>(), observed = gate<void>();
    const f = yield* fixture({ onObserve: async () => { entered.resolve(); await observed.promise; } });
    const session = yield* (yield* f.acquisition).connect;
    const old = session.bind();
    const handoff = yield* session.beginHandoff(60); f.state.text = "human changed this";
  ...
  test("resume requires the operator acknowledgement and matching token", () => Effect.gen(function* () {
    const f = yield* fixture(); const session = yield* (yield* f.acquisition).connect;
  ...
  test("selecting a tab invalidates old handles without spending an action", () => Effect.gen(function* () {
    const f = yield* fixture({ maxActions: 1 }); const session = yield* (yield* f.acquisition).connect;
    const old = session.bind(); yield* session.selectPage("page-2");
  ...
  test("keep-alive reconnect establishes a new generation and observes actual state", () => Effect.gen(function* () {
  ...
  test("persistent writer lease sees provider cleanup before its finalizer", () => Effect.gen(function* () {
    let observed = "none";
    const f = yield* fixture({ contextLease: () => Effect.succeed({ finalize: ({ cleanup }) => Effect.sync(() => {
      observed = cleanup._tag === "Some" ? cleanup.value.remote : "unknown";
    }) }) });
    const session = yield* (yield* f.acquisition).connect; yield* session.close;
    assert.equal(observed, "confirmed");
  })),
  test("native failures serialize classification without SDK secrets", () => Effect.gen(function* () {
    const owner = yield* makeOwner({ maxActions: 2, maxElapsedMillis: 1000, actionTimeoutMillis: 100 }); owner.state.phase = "open";
    const result = yield* owner.guard("fill", (ticket) => native("fill", ticket, async () => {
      ticket.dispatch(); throw new Error("PRIVATE-FILL-SECRET private-page-content wss://private-url");
  ...
  test("wall-clock movement does not spend the monotonic lifetime budget", () => Effect.gen(function* () {
    const base = yield* Clock.Clock; let wall = 1000;
    const clock: Clock.Clock = { currentTimeMillisUnsafe: () => wall, currentTimeMillis: Effect.sync(() => wall),
      currentTimeNanosUnsafe: () => BigInt(wall) * 1000000n, currentTimeNanos: Effect.sync(() => BigInt(wall) * 1000000n),
      monotonicTimeNanosUnsafe: () => base.monotonicTimeNanosUnsafe(), monotonicTimeNanos: base.monotonicTimeNanos,
      sleep: (duration) => base.sleep(duration) };
    yield* Effect.gen(function* () {
      const owner = yield* makeOwner({ maxActions: 2, maxElapsedMillis: 1000, actionTimeoutMillis: 100 }); owner.state.phase = "open";
      wall = 999999999999; assert.equal(yield* owner.guard("read", () => Effect.succeed(1)), 1);
      wall = -9999999; assert.equal(yield* owner.guard("read", () => Effect.succeed(2)), 2);
