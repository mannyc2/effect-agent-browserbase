...
export const fixture = Effect.fnUntraced(function* (options: ScriptOptions = {}) {
  ...
  const http = yield* makeHttp(
    { projectId: "project-1", apiKey: Redacted.make("PRIVATE-API-KEY") },
    fetch,
  );
  ...
  const acquisition = acquireSession(
    ...
    },
    {
      ...
      ...(options.contextLease === undefined
        ? {}
        : { context: { id: "context-1", persist: true }, contextLease: options.contextLease }),
      onCleanup: (report) =>
        Effect.sync(() => {
          reports.push(report);
        }),
