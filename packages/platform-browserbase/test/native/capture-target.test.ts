    ...
    Effect.gen(function* () {
      ...
      yield* withProvider(
        ...
        Effect.gen(function* () {
          ...
          yield* session.handle.click(BrowserClickRequest.make({ selector: "#popup" }));
          ...
          yield* popupHandle.navigate(BrowserNavigateRequest.make({ url: f.url }));
          ...
          expect((yield* session.currentHandle.pipe(Effect.result))._tag).toBe("Success");
          yield* session.close;
        }),
      );
    }),
