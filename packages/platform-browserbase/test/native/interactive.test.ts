    ...
    Effect.gen(function* () {
      ...
      yield* withProvider(
        ...
        Effect.gen(function* () {
          ...
          yield* h.fill(
            BrowserFillRequest.make({ selector: "#name", value: "typed from real CDP" }),
          );
          ...
          expect(
            (yield* h.readText(BrowserReadTextRequest.make({ selector: "#count" }))).text,
          ).toBe("1");
          ...
          const duplicate = yield* h
            .click(BrowserClickRequest.make({ selector: ".duplicate" }))
            .pipe(Effect.result);

          expect(duplicate._tag).toBe("Failure");

          const viewportPng = yield* h.screenshot(
          ...
          expect((yield* frameHandle.readText(BrowserReadTextRequest.make({}))).text).toContain(
            "frame text",
          );
          ...
          expect(
            (yield* session.clickAndWait(BrowserClickRequest.make({ selector: "#next" }))).url,
      ...
      Effect.gen(function* () {
        ...
        yield* withProvider(
          ...
          Effect.gen(function* () {
            ...
            expect(
              (yield* session.handle.readText(BrowserReadTextRequest.make({ selector: "#count" })))
                .text,
            ).toBe("0");
          }),
        );
      ...
      yield* withProvider(
        ...
        Effect.gen(function* () {
          ...
          const download = yield* session.clickForDownload(
            BrowserClickRequest.make({ selector: "#download" }),
          );
          ...
          expect(
            (yield* h.readText(BrowserReadTextRequest.make({})).pipe(Effect.result))._tag,
          ).toBe("Failure");
          expect((yield* selected.readText(BrowserReadTextRequest.make({}))).text).toContain(
            "next page",
          );
      ...
      Effect.gen(function* () {
        ...
        yield* withProvider(
          ...
          Effect.gen(function* () {
            ...
            expect(
              (yield* old
                .click(BrowserClickRequest.make({ selector: "#increment" }))
            ...
            expect(
              (yield* old.readText(BrowserReadTextRequest.make({})).pipe(Effect.result))._tag,
            ).toBe("Failure");
    ...
    Effect.gen(function* () {
      ...
      yield* withProvider(
        ...
        Effect.gen(function* () {
          ...
          const failed = yield* session.handle
            .click(BrowserClickRequest.make({ selector: "#disabled" }))
            .pipe(Effect.result);
...
const expectedCaptureError: Same<
  ...
  typeof import("@effect-agent/platform-browserbase/types").BrowserbaseError.Type
> = true;
