...
import { PageSuspension } from "@effect-agent/platform-browserbase/types";
      ...
      Effect.gen(function* () {
        ...
        yield* withProvider(
          ...
          Effect.gen(function* () {
            ...
            yield* session.handle.navigate(BrowserNavigateRequest.make({ url: `${f.url}clocks` }));
            ...
            yield* scoutHandle.navigate(
              BrowserNavigateRequest.make({ url: `${f.url}clocks#scout` }),
            );
            ...
            expect(
              (yield* heldHandle
                .click(BrowserClickRequest.make({ selector: "#click" }))
                .pipe(Effect.result))._tag,
            ).toBe("Failure");
            const resumedScout = yield* session.selectPage(secondId);

            yield* Effect.sleep(350);
            yield* resumedScout.click(BrowserClickRequest.make({ selector: "#click" }));
  ...
  Effect.scoped(
    Effect.gen(function* () {
      ...
      const invalid = yield* withProvider(
        f,
        Effect.gen(function* () {
          return yield* (yield* BrowserbaseInteractiveHost).open(policy);
        }),
        { pageControl: true, keepAlive: true },
      ).pipe(Effect.result);
