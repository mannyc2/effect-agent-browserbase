import { BrowserbaseError, PageSuspension } from "@effect-agent/platform-browserbase/types";
import { expect, it } from "@effect/vitest";

import type { Ticket } from "../src/internal/Owner.ts";
import { PageExecution, type PageExecutionNative } from "../src/internal/PageExecution.ts";
...
const fixture = (overrides: Partial<PageExecutionNative> = {}) => {
  ...
  const ticket: Ticket = {
    ...
    check: () => {
      if (aborted) throw BrowserbaseError.make({ operation: "test", reason: "stale" });
    },
    dispatch: () => {
      dispatched = true;
    },
