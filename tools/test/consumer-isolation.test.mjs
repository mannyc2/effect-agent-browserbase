...
function fixture(
  ...
  for (const item of consumerPackageSet(profile)) {
    const stage = join(directory, item.stem),
      content = join(stage, "package");

    const stems =
      item === packages[0]
        ? Object.keys(browserModules)
        : item === packages[1]
          ? ["index"]
          : ["index", "Adapter", "Tools"];

    const subpaths = stems.map((stem) =>
      stem === "index" ? "." : "./" + stem.replace(/([a-z0-9])([A-Z])/g, "$1-$2").toLowerCase(),
    );

    const exports = Object.fromEntries(
      stems.map((stem, i) => [
        subpaths[i],
        { types: `./dist/${stem}.d.mts`, default: `./dist/${stem}.mjs` },
      ]),
    );
    ...
    for (const stem of stems) {
      write(
        join(content, "dist", stem + ".mjs"),
        item === packages[0]
          ? browserModules[stem]
          : `export { owner } from "${packages[0].name}";\n`,
      );
  ...
  return {
    ...
    artifacts,
    directory,
    generic: join(root, "node_modules", packages[0].name),
    verify: () => verifyConsumer(root, artifacts, profile),
  };
}
...
function install(
  ...
  {
    exports = { ".": "./index.mjs" },
    source = "export const installed = true;\n",
    version = "1.0.0",
  } = {},
) {
  json(join(directory, "package.json"), { name, version, type: "module", exports });
  write(join(directory, "index.mjs"), source);
}

function installFramework(
  directory,
  { version = frameworkVersion, duplicate = false, directOnlyAlias = false } = {},
) {
  const modules = [
  ...
  install(directory, "effect-agent", {
    ...
    exports: {
      ".": "./index.mjs",
      ...Object.fromEntries(modules.map(([stem, subpath]) => [`./${subpath}`, `./${stem}.mjs`])),
    },
    source: modules
      .map(([stem]) => {
        if (duplicate && stem === "InteractiveBrowser")
          return "export const InteractiveBrowser = { owner: {} };";

        return directOnlyAlias
          ? `export { t as ${stem} } from "./${stem}.mjs";`
          : `export * as ${stem} from "./${stem}.mjs";`;
      })
      .join("\n"),
  });
  for (const [stem] of modules)
    write(join(directory, `${stem}.mjs`), "export const owner = {}; export const t = { owner };\n");
}
...
function resolvesInsideConsumer(root, specifier) {
  ...
  try {
    return realpathSync(createRequire(join(real, "package.json")).resolve(specifier)).startsWith(
      real + sep,
    );
  } catch (error) {
...
for (const name of [
  ...
  test(`rejects nested ${name} invisible to root resolution`, async (t) => {
    ...
    assert.equal(
      resolvesInsideConsumer(f.root, name),
      false,
      `Nested ${name} is visible to root resolution`,
    );
...
test("rejects an aliased dependency even when its root export is hidden", async (t) => {
  ...
  install(join(f.generic, "node_modules", "innocent-alias"), "effect-agent", {
...
test("rejects transitive nested dependencies before candidate code executes", async (t) => {
  ...
  const f = fixture(t, {
    source: `import { writeFileSync } from "node:fs"; writeFileSync(${JSON.stringify(marker)}, "imported"); export const owner = {};\n`,
  });
...
for (const store of [".bun", ".pnpm"]) {
  test(`accepts in-consumer ${store} links and rejects a forbidden store package`, async (t) => {
    ...
    assert.equal((await f.verify()).profile, "resources");
    install(
      join(f.root, "node_modules", store, "hidden@1", "node_modules", "alias"),
      "playwright-core",
    );
    await assert.rejects(f.verify(), /Forbidden installed dependency/);
  });
...
test("agent profile retains canonical shared-owner and retired-export checks", async (t) => {
  ...
  const adapter = await import(
    pathToFileURL(join(f.root, "node_modules", packages[2].name, "dist/Adapter.mjs")).href
  );
...
for (const profile of ["browser", "agent"]) {
  test(`${profile} refuses Browserbase under a nested alias`, async (t) => {
    ...
    install(join(f.generic, "node_modules", "provider-alias"), "effect-browserbase", {
      exports: { "./hidden": "./index.mjs" },
    });
...
test("hosted Agent integration keeps all three packages on the same browser owner", async (t) => {
  ...
  for (const item of packages.slice(1)) {
    const consumer = await import(
      pathToFileURL(join(f.root, "node_modules", item.name, "dist/index.mjs")).href
    );
...
test("rejects a duplicate provider receipt issuer hidden beneath an alias", async (t) => {
  const f = fixture(t, { profile: "agent-hosted" });

  install(join(f.generic, "node_modules/provider-alias"), "effect-browserbase", { version });
  await assert.rejects(f.verify(), /Multiple installed effect-browserbase registries/);
});
